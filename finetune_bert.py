...
parser.add_argument('--bert_init', type=str, default='roberta-base',
                    choices=['roberta-base', 'emilyalsentzer/Bio_ClinicalBERT', 'dmis-lab/biobert-base-cased-v1.2', 'microsoft/BiomedNLP-PubMedBERT-base-uncased-abstract', 'allenai/scibert_scivocab_uncased', 'bert-base-cased'])
...
corpus_file = './data/'+dataset+'_shuffle.txt'
...
def train_step(engine, batch):
    ...
    weight = th.tensor([4.0,6.0])
    weight = weight.to(gpu)
    loss = F.cross_entropy(y_pred, y_true, weight)
    loss.backward()
    optimizer.step()
