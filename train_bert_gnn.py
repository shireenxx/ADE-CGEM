...
parser.add_argument('--bert_init', type=str, default='roberta-base',
                    choices=['roberta-base', 'emilyalsentzer/Bio_ClinicalBERT', 'dmis-lab/biobert-base-cased-v1.2', 'allenai/scibert_scivocab_cased', 'microsoft/BiomedNLP-PubMedBERT-base-uncased-abstract', 'allenai/scibert_scivocab_uncased', 'bert-base-cased'])
parser.add_argument('--pretrained_bert_ckpt', default=None)
parser.add_argument('--dataset', default='adr', choices=['smm4h','smm4h_l','twimed_pub','twimed_twi','cadec','adr'])
...
parser.add_argument('--gcn_model', type=str, default='gat', choices=['gcn', 'gat'])
...
corpse_file = './data/' + dataset +'_shuffle.txt'
...
logger.info(str(g))
print('built Gragh successfully')
...
def log_training_results(trainer):
    ...
    train_acc, train_p, train_r, train_nll = metrics["acc"], metrics["p"], metrics["r"], metrics["nll"]
    train_f = 2 * train_p[1].item() * train_r[1].item() / (train_p[1].item() + train_r[1].item())
    evaluator.run(idx_loader_val)
    metrics = evaluator.state.metrics
    val_acc, val_p, val_r, val_nll = metrics["acc"], metrics["p"], metrics["r"], metrics["nll"]
    #val_f = 2 * val_p[1].item() * val_r[1].item() / (val_p[1].item() + val_r[1].item())
    val_f = 0
    evaluator.run(idx_loader_test)
    metrics = evaluator.state.metrics
    test_acc, test_p, test_r, test_nll = metrics["acc"], metrics["p"], metrics["r"], metrics["nll"]
    test_f = 2 * test_p[1].item() * test_r[1].item() / (test_p[1].item() + test_r[1].item())
    logger.info(
        "\rEpoch: {}  Train acc: {:.4f} nonADR_precision: {:.4f} ADR_precision {:.4f} nonADR_recall: {:.4f}, ADR_recall: {:.4f}, F1_score: {:.4f}, loss: {:.4f}  Val acc: {:.4f} nonADR_precision: {:.4f} ADR_precision {:.4f} nonADR_recall: {:.4f}, ADR_recall: {:.4f}, F1_score: {:.4f}, loss: {:.4f}  Test acc: {:.4f} nonADR_precision: {:.4f} ADR_precision {:.4f} nonADR_recall: {:.4f}, ADR_recall: {:.4f}, F1_score: {:.4f}, loss: {:.4f}"
        .format(trainer.state.epoch, train_acc, train_p[0].item(), train_p[1].item(), train_r[0].item(), train_r[1].item(), train_f, train_nll, val_acc, val_p[0].item(), val_p[1].item(), val_r[0].item(), val_r[1].item(), val_f, val_nll, test_acc, test_p[0].item(), test_p[1].item(), test_r[0].item(), test_r[1].item(), test_f, test_nll)
    )
